...
**对data**，weight blob大小为Number x Channels x Height x Width, 如256x3x224x224；

**对conv层**，weight blob大小为 Output 节点数 x Input 节点数 x Height x Width，如AlexNet第一个conv层的blob大小为96 x 3 x 11 x 11；  

**对inner product(全连接) 层**， weight blob大小为 1 x 1 x Output节点数 x Input节点数； bias blob大小为1 x 1 x 1 x Output节点数（ conv层和inner product层一样，也有weight和bias（偏移），所以在网络结构定义中我们会看到两个blobs_lr，第一个是weights的，第二个是bias的。类似地，weight_decay也有两个，一个是weight的，一个是bias的）；
